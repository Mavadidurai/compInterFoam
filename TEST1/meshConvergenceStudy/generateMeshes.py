#!/usr/bin/env python3
"""
Generate blockMeshDict files for mesh convergence study - TEST1 (Uniform Mesh)

This script creates multiple refinement levels based on TEST1's uniform mesh configuration.
TEST1 uses simpleGrading (1 1 1) throughout - no mesh grading.

Refinement levels:
- coarse:    0.5x (40×200×20 substrate, 40×200×30 air, 40×200×18 Ti)
- medium:    1.0x (80×400×40 substrate, 80×400×60 air, 80×400×36 Ti) [baseline]
- fine:      1.5x (120×600×60 substrate, 120×600×90 air, 120×600×54 Ti)
- very_fine: 2.0x (160×800×80 substrate, 160×800×120 air, 160×800×72 Ti)
"""

import os
import sys

# Base mesh configuration from TEST1 (uniform mesh)
BASE_CONFIG = {
    'substrate': {'Nx': 80, 'Ny': 400, 'Nz': 40, 'grading': '(1 1 1)'},
    'air_gap': {'Nx': 80, 'Ny': 400, 'Nz': 60, 'grading': '(1 1 1)'},
    'ti_film': {'Nx': 80, 'Ny': 400, 'Nz': 36, 'grading': '(1 1 1)'}
}
...
// Generated for mesh convergence study - TEST1 UNIFORM MESH
...
blocks
(
    // Substrate (receiver) - UNIFORM grading
    hex ( 0  3  2  1   4  7  6  5)   ({sub_nx} {sub_ny} {sub_nz})  simpleGrading {sub_grading}

    // Air gap - UNIFORM grading
    hex ( 4  7  6  5   8 11 10  9)   ({air_nx} {air_ny} {air_nz})  simpleGrading {air_grading}

    // Ti film (71.4 nm) - UNIFORM grading
    hex ( 8 11 10  9  12 15 14 13)   ({ti_nx} {ti_ny} {ti_nz})  simpleGrading {ti_grading}
);
...
// Substrate: {sub_dx:.3f} × {sub_dy:.3f} × {sub_dz:.3f} μm
// Air gap:   {air_dx:.3f} × {air_dy:.3f} × {air_dz:.3f} μm
// Ti film:   {ti_dx:.3f} × {ti_dy:.3f} × {ti_dz:.4f} μm ({ti_dz_nm:.2f} nm in z)
...
def generate_mesh(level, factor, output_dir):
    ...
    # Calculate cell sizes (domain dimensions)
    ...
    print(f"  Substrate: {sub_nx}×{sub_ny}×{sub_nz}")
    print(f"  Air gap:   {air_nx}×{air_ny}×{air_nz}")
    print(f"  Ti film:   {ti_nx}×{ti_ny}×{ti_nz} (z-cell: {ti_dz_nm:.2f} nm)")
    print()
...
def main():
    ...
    print("="*70)
    print("Mesh Convergence Study - TEST1 (Uniform Mesh)")
    print("="*70)
    print()
